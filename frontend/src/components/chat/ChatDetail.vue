  ...
  <div class="chat-detail">
    ...
    <div class="chat-header">
      <div class="chat-header-left">
        <n-avatar
          :src="store.currentConversation?.avatar"
          round
          size="medium"
        />
        <div class="chat-info">
          <div class="chat-name">
            {{ store.currentConversation?.username }}
          </div>
...
import {computed, nextTick, onMounted, onUnmounted, ref, watch, h, getCurrentInstance} from 'vue'
...
import { NIcon, NDropdown, NAvatar, NSpin, NEmpty, NText, useMessage } from 'naive-ui'
...
const sendReadReceiptIfNeed = (userId: number, reason = '检查未读消息') => {
    if (!userId) {
        return
    }
...
onMounted(async () => {
  ...
  if (routeUserId || !store.currentConversation) {
    ...
    if (savedSession) {
      ...
      message.success(`已恢复到与 ${savedSession.username} 的聊天`)
