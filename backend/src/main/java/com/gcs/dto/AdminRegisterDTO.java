 ...
 */
@Data
@Schema(description = "管理员注册请求对象")
public class AdminRegisterDTO {
    
    @NotBlank(message = "账号不能为空")
    @Schema(description = "管理员账号", required = true, example = "admin123")
  private String account;
    
    @NotBlank(message = "密码不能为空")
    @Size(min = 6, max = 20, message = "密码长度必须在 6-20 位之间")
    @Pattern(regexp = "^[a-zA-Z0-9]+$", message = "密码只能包含字母和数字")
    @Schema(description = "管理员密码", required = true, example = "Admin@123")
  ...
  private String phone;
    ...
    @Schema(description = "管理员昵称（可选，为空则使用账号名）", example = "管理员")
